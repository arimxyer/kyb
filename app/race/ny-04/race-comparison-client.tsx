...
import { formatCurrency } from "@/lib/voter-types";

export function RaceComparisonClient() {
  const data = useQuery(api.ballot.getRaceComparison, {
    raceSlug: "ny-04",
  });
    ...
    return (
      <main className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-2xl items-center px-4 py-16 sm:px-6">
        <Card className="w-full border-border/90 shadow-sm">
          <CardHeader>
            <CardTitle>Comparison data is being prepared</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm leading-6 text-muted-foreground">
              The NY-04 dataset has not finished loading. Please check again
              shortly.
            </p>
            <Link
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div className="mt-5 max-w-3xl">
            ...
            <h1 className="mt-4 text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
              Compare candidates for {race.office}
            </h1>
            <p className="mt-3 text-lg text-muted-foreground">
              {race.districtLabel} · {election.name}
            </p>
          </div>
        </div>
      </section>
